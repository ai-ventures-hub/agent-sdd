...
| `--update`  | Apply a UI/UX tweak or fix (includes tests, auto style fixes) | Target file/component        |
...
5. **--update**: Executes `workflows/update.md` to apply UI/UX tweaks or fixes to a target file/component, including tests and style fixes.
6. **--review**: Executes `workflows/review.md` to check modified paths against `.claude/standards/theme-standards.md` using `.claude/agents/code-reviewer.md`.
7. **--analyze**: Executes `workflows/analyze.md` to scan the project for issues and suggest improvements.
8. **--check**: Executes `workflows/check.md` to verify task completion against `tasks.json` using `.claude/agents/task-schema-validator.md`.

## Task Schema Validation
For flags involving tasks (`--spec`, `--execute`, `--update`, `--check`):
...
2. **Validate Task (if applicable)**: For `--spec`, `--execute`, `--update`, `--check`, run `.claude/agents/task-schema-validator.md` to ensure valid `tasks.json`.
