...
3. If --no-tests, skip all test writing and execution; else, write TDD tests for critical components and run full test suite (per .agent-sdd/standards/best-practices.md).
4. Prompt user to confirm target file path (e.g., components/FileWatcher/FileWatcher.tsx for UI tasks, lib/fileMonitor.js for utilities).
5. Implement code and save changes to confirmed path, ensuring WCAG 2.1 AA compliance.
6. Prompt for commit:
...
8. If --no-tests or --quick/--no-spec, skip all test execution; else, run task-specific tests (if package.json exists).
...
11. Notify user with summary: tasks completed, issues, test results (if any), files to commit in GitHub Desktop.